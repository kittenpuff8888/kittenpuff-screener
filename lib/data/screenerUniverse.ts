...
export const SETUPS: SetupDef[] = [
  ...
  },
  {
    ...
    icon: "⤢",
    hasBear: true,
    req: "Regular RSI(14, Wilder-smoothed) divergence confirmed today — a literal port of a user-supplied Pine Script v6 divergence indicator, comparing the two most recent confirmed 5-bar RSI pivots, no RSI-value threshold. Bull: price (Low) makes a lower low while RSI makes a higher low. Bear: price (High) makes a higher high while RSI makes a lower high.",
    bull: (r) => r.rsiDivBullish,
    bear: (r) => r.rsiDivBearish,
  },
  {
    key: "rsi_divergence_hidden",
    label: "RSI Hidden Divergence",
    icon: "⤡",
    hasBear: true,
    req: "Hidden RSI(14, Wilder-smoothed) divergence confirmed today — the exact mirror of RSI Divergence above, same 5-bar confirmed-pivot logic, no RSI-value threshold. Bull: price (Low) makes a higher low while RSI makes a lower low. Bear: price (High) makes a lower high while RSI makes a higher high.",
    bull: (r) => r.rsiDivHiddenBullish,
    bear: (r) => r.rsiDivHiddenBearish,
  },
  {
